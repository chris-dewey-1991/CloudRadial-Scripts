# —–––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––
# CloudRadial Data Agent - Check Script
# —–––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––
...
$folderCandidates = @(
    ...
    'C:\Program Files\CloudRadial Agent'
)

$foundFolder = $folderCandidates | Where-Object { Test-Path -Path $_ -PathType Container } | Select-Object -First 1

if ($foundFolder) {
    Add-Result -Name "Folder Exists: CloudRadial Agent" -Passed $true -Details "(Found: $foundFolder)"
} else {
    Add-Result -Name "Folder Exists: CloudRadial Agent" -Passed $false -Details "(Not found in either Program Files or Program Files (x86))"
}

# -------------------------
# 3) Service Check
#    Exact "CloudRadial" OR best match like CloudRadial*
# -------------------------
$targetServiceName = 'CloudRadial'

$svc = Get-Service -Name $targetServiceName -ErrorAction SilentlyContinue

if (-not $svc) {
    $svc = Get-Service -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -like 'CloudRadial*' -or $_.DisplayName -like 'CloudRadial*' } |
        Select-Object -First 1
}

if (-not $svc) {
    Add-Result -Name "Service Present: CloudRadial (or CloudRadial*)" -Passed $false -Details "(No matching service found)"
} else {
...
# -------------------------
Write-Output ""
Write-Output "============== CloudRadial Data Agent =============="
...
if ($fails.Count -eq 0) {
    Write-Output "[SUCCESS] CloudRadial Data Agent is installed and running."
    #exit 0
} else {
